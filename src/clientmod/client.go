...
func main() {
	//fmt.Println("Client modified dfdsfsdfds!!!!!")
...
func putCommandClassicPaxos(writers []*bufio.Writer, readers []*bufio.Reader, leader int) {
	var id int32 = 10
	args := genericsmrproto.Propose{id, state.Command{state.PUT, 12, 30}, time.Now().UnixNano()}
	writers[leader].WriteByte(genericsmrproto.PROPOSE)
	args.Marshal(writers[leader])
	writers[leader].Flush()
	fmt.Println("Classic paxos: Command PUT success")
	waitReply(readers, leader)
}

func putCommandFastPaxos(writers []*bufio.Writer, readers []*bufio.Reader, leader int) {
	var id int32 = 10
	args := genericsmrproto.Propose{id, state.Command{state.PUT, 12, 30}, time.Now().UnixNano()}
	//send to everyone
	for rep := 0; rep < N; rep++ {
	...
	fmt.Println("Fast paxos: Command PUT success")
	waitReply(readers, leader)
}

//func putCommandEpaxos(writers []*bufio.Writer, readers []*bufio.Reader, leader int) {
//	var id int32 = 10
//	args := genericsmrproto.Propose{id, state.Command{state.PUT, 12, 30}, time.Now().UnixNano()}
//	if *noLeader {
//		leader = rarray[i]
...
//	writers[leader].WriteByte(genericsmrproto.PROPOSE)
//	args.Marshal(writers[leader])
//	fmt.Println("Epaxos: Command PUT success")
//	waitReply(readers, leader)
//}


func getCommandClassicPaxos(writers []*bufio.Writer, readers []*bufio.Reader, leader int) {
	var id int32 = 11
	args := genericsmrproto.Propose{id, state.Command{state.GET, 12, 1}, time.Now().UnixNano()}
	writers[leader].WriteByte(genericsmrproto.PROPOSE)
	args.Marshal(writers[leader])
	writers[leader].Flush()
	fmt.Println("Command GET success")
	//fmt.Println()
	waitReply(readers, leader)
}

func waitReply(readers []*bufio.Reader, leader int) {
